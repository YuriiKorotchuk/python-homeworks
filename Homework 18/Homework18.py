...
class SchoolClass:
    ...
    def pupils_quantity(self):
        number_of_pupils = 0
        for i in self.pupils:
            number_of_pupils += 1
        return number_of_pupils
...
class Staff(ABC):
    def __init__(self, name: str, surname: str, age: datetime.date, school):
        self.name = name
...
class Teacher(Staff):
    ...
    def __init__(self, name, surname, age: datetime.date, school):
...
class Director(Staff):
    ...
    @staticmethod
    def invite_new_pupil(school_class: SchoolClass, pupil):
        if pupil in school_class.pupils:
            return f"This pupil already in the class"
        else:
            school_class.pupils.append(pupil)
    ...
    def fire_pupil(school_class, pupil):
        school_class.pupils.remove(pupil)
